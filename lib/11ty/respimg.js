...
/**
 *
 * @param {string[]} globs - Globs of additional images to optimize and move
 *
...
async function respimg(content, outputPath) {
  ...
  (await glob(config.images.additional.map(i => path.join(config.folders.source, i))))
    .map(src => ({
      src,
      buff: readFileSync(src),
    }))
    .forEach(f => {
...
module.exports.setup = (eleventy, c) => {
  ...
  if (eleventy.addWatchTarget && config.images.watch) {
    if (Array.isArray(config.images.watch)) {
      config.images.watch.map(p => eleventy.addWatchTarget(path.join(config.folders.source, p)));
    } else {
      eleventy.addWatchTarget(path.join(config.folders.source, config.images.watch));
    }
  }
};
