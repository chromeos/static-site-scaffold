    ...
    async generateBundle(opts, bundle) {
      ...
      while (bundles.length) {
        ...
        if (bndl.code.match(globalRegexp)) {
          ...
          bndl.code = source;
          bndl.map = map;
          const warn = `\nThere ${warnings.length > 1 ? 'were' : 'was'} ${chalk.red(warnings.length)} ${chalk.red(warnings.length > 1 ? 'warnings' : 'warning')}.`;
          console.log(`Injected ${count} files for precaching, ${chalk.yellow(size * 0.001 + 'KB')} total, into chalk.cyan(${bndl.fileName}).${warnings.length ? warn : ''}`);
          if (warnings.length) {
